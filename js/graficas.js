...
$(document).ready(function(){
  barra("Nintendo","Action");
    ...
    barra3("Nintendo","Action");
    $("input[type='button']").click(function(){
            var radioValue = $("input[name='plataforma3']:checked").val();
            if(radioValue){
                $(document).on('change','#genero2006',function(){
                  var valor = $('#genero2006>option:selected').text();
                  barra3(radioValue,valor);
                });
            }
    }); 
});
...
function graphLineSec4(){
  var linea1 = dc.compositeChart("#ventascomp_sony_nin_sega_mic_2006");
  d3.csv("data/06-16/nintendo_sega_sony_mic_0616.csv").then(function(experiments) {
  ...
  linea1
      .width(768)
      .height(480)
      .x(d3.scaleLinear().domain([2006,2016]))
      .legend(dc.legend().x(80).y(20).itemHeight(13).gap(5))
      .renderHorizontalGridLines(true) 
      .dimension(dimension)
    .compose([
  ...
  var linea2 = dc.compositeChart("#ventascomp_sony_nin_sega_mic_jap_2006");
  d3.csv("data/06-16/nintendo_sega_sony_mic_0616_jap.csv").then(function(experiments) {
...
function barra3(json,valor){
  var barra_nintendo_sec2 = dc.barChart('#graph_barras_ventas_genero_sec4');
  console.log("Este es el valor del select: " + valor + "y el valor del json: " +json);
  d3.json('data/06-16/'+json+'_0616_ventas_generos.json').then(function(counts) {
      var ndx            = crossfilter(counts),
          fruitDimension = ndx.dimension(function(d) {return d.Year;});
